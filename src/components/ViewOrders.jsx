...
function ViewOrders() {
    ...
    const fetchOrders = async () => {
        try {
            setLoading(true);
            // Since we don't have orders in the database yet, we'll create some mock data
            // In a real application, this would fetch from an orders endpoint
            const mockOrders = [
                {
                    id: "ORD001",
                    ...
                    items: [
                        { productId: "4b60", name: "iPhone 17 Pro", quantity: 1, price: 1099.99 },
                        { productId: "e005", name: "AirPods Pro 2nd Gen", quantity: 1, price: 249.99 }
                    ],
                    totalAmount: 1349.98,
                    status: "pending",
                ...
                },
                {
                    ...
                    items: [
                        { productId: "c001", name: "Classic White Dress Shirt", quantity: 2, price: 89.99 },
                        { productId: "c006", name: "Dark Wash Jeans", quantity: 1, price: 79.99 }
                    ],
                    totalAmount: 259.97,
                    status: "shipped",
                ...
                },
                {
                    ...
                    items: [
                        { productId: "e002", name: "Sony WH-1000XM5 Headphones", quantity: 1, price: 399.99 }
                    ],
                    totalAmount: 399.99,
                    status: "delivered",
                ...
                <div className="glass-panel p-6 animate-[fadeIn_0.8s_ease-out]">
                    ...
                    {orders.length === 0 ? (
                        <div className="text-center py-16">
                            ...
                            <p className="text-text/60 mb-6">Orders will appear here when customers make purchases</p>
                        </div>
                    ) : (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="relative flex items-center justify-between mb-6">
                                            ...
                                            <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium border transition-all duration-300 ${getStatusColor(order.status)}`}>
                                        ...
                                        <div className="mb-4 p-3 rounded-lg bg-card/20 border border-border/20 group-hover:border-primary/20 transition-colors duration-300">
                                            <p className="text-text/80 text-sm font-medium mb-1">{order.customerName}</p>
                                            <p className="text-text/60 text-xs">{order.customerEmail}</p>
                                        </div>

                                        {/* Order Items Preview */}
                                        <div className="flex-1 mb-4">
                                            <h4 className="text-text/70 text-sm font-medium mb-3">Order Items ({order.items.length})</h4>
                                            <div className="space-y-2 min-h-[60px]">
                                                {order.items.slice(0, 2).map((item, itemIndex) => (
                                                    <div key={itemIndex} className="flex items-center justify-between text-xs">
                                                        <span className="text-text/70 truncate flex-1 mr-2">{item.name}</span>
                                                        <span className="text-text/50">x{item.quantity}</span>
                                                    </div>
                                                ))}
                                                {order.items.length > 2 && (
                                                    <p className="text-text/50 text-xs italic">+{order.items.length - 2} more items</p>
                                                )}
                                        ...
                                        <div className="border-t border-border/30 pt-4 mb-4 group-hover:border-primary/20 transition-colors duration-300">
                                            <div className="flex items-center justify-between">
                                                <span className="text-text/60 text-sm">Total Amount</span>
                            ...
                            <div className="relative space-y-4">
                                ...
                                <div className="bg-card/40 rounded-xl p-4 border border-border/30 hover:border-primary/20 transition-colors duration-300">
                                    ...
                                    <div className="space-y-2">
                                        <p className="text-text/90 flex items-center gap-2">
                                            <span className="font-medium text-primary min-w-[60px]">Name:</span>
                                            <span>{selectedOrder.customerName}</span>
                                        </p>
                                        <p className="text-text/90 flex items-center gap-2">
                                            <span className="font-medium text-primary min-w-[60px]">Email:</span>
                                            <span>{selectedOrder.customerEmail}</span>
                                        </p>
                                        <p className="text-text/90 flex items-start gap-2">
                                            <span className="font-medium text-primary min-w-[60px] mt-1">Address:</span>
                                ...
                                <div className="bg-card/40 rounded-xl p-4 border border-border/30 hover:border-primary/20 transition-colors duration-300">
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <h3 className="text-lg font-bold text-white">Order Items ({selectedOrder.items.length})</h3>
                                    </div>
                                    <div className="space-y-3">
                                        {selectedOrder.items.map((item, index) => (
                                            <div key={index} className="bg-card/60 rounded-lg p-3 border border-border/20 hover:border-primary/20 transition-colors duration-300">
                                                <div className="flex justify-between items-start">
                                                    <div className="flex-1">
                                                        <p className="font-medium text-white mb-1">{item.name}</p>
                                                        <p className="text-text/60 text-sm">Quantity: {item.quantity}</p>
                                                    </div>
                                                    <div className="text-right">
